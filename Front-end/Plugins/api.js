...
export default (context, inject) => {
  inject("query", (resource, params) => {
    return new Promise((resolve, reject) => {
      getAxios(context).get(`${resource}`, {params}).then(
        (data) => {
          resolve(data);
        }).catch(error => {
        handleError(error);
  ...
  inject("get", (resource, slug = "", params = null) => {
    return new Promise((resolve, reject) => {
      getAxios(context).get(`${resource}/${slug}`, {params}).then(
        (data) => {
          resolve(data);
        }).catch(error => {
        handleError(error);
  ...
  inject("post", (resource, params) => {
    return new Promise((resolve, reject) => {
      getAxios(context).post(`${resource}`, params).then(data => {
        resolve(data);
      }).catch(error => {
        handleError(error);
        reject(error);
  ...
  inject("update", (resource, slug, params) => {
    return new Promise((resolve, reject) => {
      getAxios(context).$axios.put(`${resource}/${slug}`, params).then(
        (data) => {
          resolve(data.data);
        }).catch(error => {
        handleError(error);
  ...
  inject("put", (resource, params) => {
    return new Promise((resolve, reject) => {
      getAxios(context).put(`${resource}`, params).then(
        (data) => {
          resolve(data.data);
        }).catch(error => {
        handleError(error);
  ...
  inject("delete", (resource) => {
    return new Promise((resolve, reject) => {
      getAxios(context).delete(resource).then(
        (data) => {
          resolve(data.data);
        }).catch(error => {
        handleError(error);
  ...
  function configRequest(context) {
    ...
    if (token) {
      context.app.$axios.defaults.headers.common[
              'Authorization'
              ] = `Bearer ${token}`
    }
  }
  ...
  function handleError(error) {
    if (error && error.response) {
      ...
      if (statusCode === 500) {
        ...
        context.store.dispatch(`alert/${ERROR}`, error.response.data.message, {root: true});
